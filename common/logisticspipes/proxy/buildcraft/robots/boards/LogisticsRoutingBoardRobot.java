...
public class LogisticsRoutingBoardRobot extends RedstoneBoardRobot {
	...
	private EnumFacing targetStationSide = EnumFacing.UNKNOWN;
	...
	public LPTravelingItemServer handleItem(LPTravelingItemServer arrivingItem) {
		...
		ITransactor trans = InventoryHelper.getTransactorFor(robot, EnumFacing.UNKNOWN);
		ItemStack inserted = trans.add(arrivingItem.getItemIdentifierStack().makeNormalStack(), EnumFacing.UNKNOWN, false);
		if (inserted.stackSize != arrivingItem.getItemIdentifierStack().getStackSize()) {
			acceptsItems = false;
			startTransport();
			return arrivingItem;
		}
		inserted = trans.add(arrivingItem.getItemIdentifierStack().makeNormalStack(), EnumFacing.UNKNOWN, true);
		if (inserted.stackSize != arrivingItem.getItemIdentifierStack().getStackSize()) {
