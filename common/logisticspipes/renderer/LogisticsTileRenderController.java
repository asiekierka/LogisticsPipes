		...
		}
		{
			...
			while (iter.hasNext()) {
				...
				if (data.timeout < 0 || data.isDeadEntity()) {
					data.setDead();
					if (data.sendPacket()) {
						MainProxy.sendPacketToAllWatchingChunk(pipe.getX(), pipe.getZ(), MainProxy.getDimensionForWorld(pipe.getWorld()), PacketHandler.getPacket(PowerPacketLaser.class).setColor(key).setRenderBall(true).setDir(EnumFacing.UNKNOWN).setRemove(true).setTilePos(pipe));
					}
					iter.remove();
				}
			}
